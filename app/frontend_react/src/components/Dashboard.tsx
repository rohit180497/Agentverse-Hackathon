...
export const Dashboard = ({ tripInfo }: DashboardProps) => {
  const [loadingStates, setLoadingStates] = useState({
    ...
    restaurants: true,
    events: true,
    summary: true // ✅ Added summary to loadingStates
  });
  ...
  useEffect(() => {
    const loadingTimers = [
      ...
      { key: 'summary', delay: 2800 } // ✅ Added summary delay
    ];
  ...
  return (
    <div className="h-screen overflow-y-auto p-6 scrollbar-none">
      ...
      <div className={`animate-scale-in mt-6 ${loadingStates.summary ? 'opacity-0' : 'opacity-100'}`} style={{transitionDelay: '700ms'}}>
      <SummaryCard destination={tripInfo.destination} />
      </div>
